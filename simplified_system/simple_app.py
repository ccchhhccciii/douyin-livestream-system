...
class SimpleApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.logger.info("简化版直播系统UI初始化完成")
    ...
    def _setup_logging(self):
        ...
        log_file = os.path.join(log_dir, f"simple_app_{timestamp}.log")
    ...
    def _on_custom_message_clicked(self):
        ...
        if ok and text.strip():
            # 使用线程添加消息到系统，避免UI阻塞
            self.message_thread = MessageThread(self.stream_system, text)
            self.message_thread.message_completed.connect(self._on_message_completed)
            self.message_thread.message_error.connect(self._on_message_error)
            self.message_thread.start()
    
    def _on_message_completed(self, success):
        """消息处理线程完成回调
    ...
    def _on_message_error(self, error_message):
        ...
        Args:
            error_message: 错误信息
        """
        self.logger.error(f"处理消息时出错: {error_message}")
        self.log_message(f"处理消息时出错: {error_message}")
    ...
    def _on_add_script_clicked(self):
        ...
        if ok and text.strip():
            # 使用线程添加话术到系统，避免UI阻塞
            self.script_thread = ScriptThread(self.stream_system, text)
            self.script_thread.script_completed.connect(self._on_script_completed)
            self.script_thread.script_error.connect(self._on_script_error)
    ...
    def _on_script_completed(self, success):
        ...
        """
        if not success:
            self.log_message("话术处理失败")
    
    def _on_script_error(self, error_message):
        """话术处理线程错误回调
        
        Args:
            error_message: 错误信息
        """
        self.logger.error(f"处理话术时出错: {error_message}")
        self.log_message(f"处理话术时出错: {error_message}")
...
class MessageThread(QThread):
    """消息处理线程，避免消息处理阻塞UI"""
    message_completed = pyqtSignal(bool)  # 消息处理完成信号，参数为是否成功
    message_error = pyqtSignal(str)       # 消息处理错误信号，参数为错误信息
    
    def __init__(self, stream_system, message):
        """初始化线程
    ...
    def run(self):
        ...
        try:
            # 添加消息到系统
            success = self.stream_system.add_custom_message(self.message)
            self.message_completed.emit(success)
        except Exception as e:
            self.logger.error(f"消息处理线程出错: {e}", exc_info=True)
...
class ScriptThread(QThread):
    """话术处理线程，避免话术处理阻塞UI"""
    script_completed = pyqtSignal(bool)  # 话术处理完成信号，参数为是否成功
    script_error = pyqtSignal(str)       # 话术处理错误信号，参数为错误信息
    
    def __init__(self, stream_system, script):
        """初始化线程
    ...
    def run(self):
        ...
        try:
            # 添加话术到系统
            success = self.stream_system.add_custom_script(self.script)
            self.script_completed.emit(success)
        except Exception as e:
            self.logger.error(f"话术处理线程出错: {e}", exc_info=True)
